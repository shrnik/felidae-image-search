"""
Compute DINOv3 ViT-B/16 embeddings for the Felidae Conservation Fund camera trap dataset.

Model: facebook/dinov3-vitb16-pretrain-lvd1689m
Embeddings are extracted from the CLS token of the final hidden state and L2-normalized.

Usage:
    # Process from locally downloaded images (faster, recommended):
    python compute_embeddings_dinov3.py --mode local --image-dir /path/to/images

    # Resume an interrupted run:
    python compute_embeddings_dinov3.py --mode local --image-dir /path/to/images --resume

    # Sanity check — embed the first image and print dimension/norm:
    python compute_embeddings_dinov3.py --mode local --image-dir /path/to/images --dry-run

    # Stream images from Azure (no local disk needed):
    python compute_embeddings_dinov3.py --mode stream
"""
...
def embed_batch(
    ...
    # CLS token from the final hidden state
    cls = outputs.last_hidden_state[:, 0, :]
    cls = cls / cls.norm(p=2, dim=-1, keepdim=True)
